...
class DecoupledSOLOHead(nn.Module):
    ...
    def forward(self, feats, eval=0):
        ...
        ins_pred_x, ins_pred_y, cate_pred, \
                ins_pred_x_eval, ins_pred_y_eval, cate_pred_eval = multi_apply(self.forward_single, new_feats,
                                                        list(range(len(self.seg_num_grids))),
                                                        eval=eval, upsampled_size=upsampled_size)
        predictions = {
                'train_tensors':{'ins_preds_x': ins_pred_x, 'ins_preds_y': ins_pred_y, 'cate_preds': cate_pred},
                'eval_tensors': {'seg_preds_x': ins_pred_x_eval, 'seg_preds_y': ins_pred_y_eval, 'cate_preds':cate_pred_eval},
        }
        return predictions
    ...
    def forward_single(self, x, idx, eval=0, upsampled_size=None):
        ins_feat = x
        cate_feat = x
        # ins branch
        # concat coord
        ...
        if eval==2:  # EndtoEnd with Tracking
            ins_pred_x_eval = F.interpolate(ins_pred_x.clone().sigmoid(), size=upsampled_size, mode='bilinear')
            ins_pred_y_eval = F.interpolate(ins_pred_y.clone().sigmoid(), size=upsampled_size, mode='bilinear')
            cate_pred_eval = points_nms(cate_pred.clone().sigmoid(), kernel=2).permute(0, 2, 3, 1)
            return ins_pred_x, ins_pred_y, cate_pred, ins_pred_x_eval.detach(), \
                            ins_pred_y_eval.detach(), cate_pred_eval.detach()
        elif eval==1:  # Evaluation Mode
            ...
            return [], [], [], ins_pred_x.detach(), ins_pred_y.detach(), cate_pred.detach()
        else:
    ...
    def loss(self,
        ...
        if len(loss_ins) == 0:
            return dict(loss_ins=torch.tensor([0.0]).cuda(), loss_cate=torch.tensor([0.0]).cuda())
    ...
    def get_seg_single(self,
        ...
        seg_dict = {'masks': seg_masks, 'labels': cate_labels, 'scores': cate_scores}
        return seg_dict
