...
class TwoStreamResNet(nn.Module):
    def __init__(self,
                 ...
                 with_cp=False,
                 zero_init_residual=True,
                 sensor_fusion_type='concat',
                 sensor_fusion_kernel=None):

        super(TwoStreamResNet, self).__init__()
        resnet_inputs = (depth, in_channels, num_stages, strides, dilations, out_indices, style,
            frozen_stages, conv_cfg, norm_cfg, norm_eval, dcn, stage_with_dcn, gcb,
            stage_with_gcb, gen_attention, stage_with_gen_attention, with_cp, zero_init_residual)

        self.appearance_stream = ResNet(*resnet_inputs)
        self.motion_stream = ResNet(*resnet_inputs)

        self.sensor_fusion_kernel = sensor_fusion_kernel
        self.sensor_fusion_type = sensor_fusion_type
    ...
    def sensor_fusion(self, appearance_features, motion_features, level_idx):
        if self.sensor_fusion_type == 'concat':
            fused = torch.cat((appearance_features, motion_features), dim=1)
        elif self.sensor_fusion_type == 'patchwise_att':
            if level_idx > 1:
                motion_features = self.bottleneck_layers[level_idx-2](motion_features)
                appearance_features = self.bottleneck_layers[level_idx-2](appearance_features)

            fused = self.sta_module[level_idx](motion_features, appearance_features)
            if level_idx > 1:
                fused = self.reverse_bottleneck_layers[level_idx-2](fused)
        return fused
