...
#                p = workers.apply_async(BaseDataset._pq_compute_single_core, (proc_id, gt_jsons_set, pred_jsons_set, gt_pans_set, pred_pans_set, gt_image_jsons_set, categories))
                pq_stat += BaseDataset._pq_compute_single_core(proc_id, gt_jsons_set, pred_jsons_set, gt_pans_set, pred_pans_set, gt_image_jsons_set, categories)
...
#                pq_stat += p.get()
    ...
    def get_unified_pan_result(self, segs, pans, cls_inds, stuff_area_limit=4 * 64 * 64, names=None, num_seg_classes=19, num_classes=9):

        pred_pans_2ch = {}
        for (seg, pan, cls_ind, name) in zip(segs, pans, cls_inds,names):
            pan_seg = pan.copy()
            pan_ins = pan.copy()
            id_last_stuff = num_seg_classes - num_classes # 10
            ids = np.unique(pan)
