...
class CityscapesVps(BaseDataset):
    ...
    def __init__(self, panoptic_json_file='', panoptic_gt_folder=''):
        ...
        self.lambda_=5
        self.labeled_fid=20
        self.panoptic_json_file = panoptic_json_file
        self.panoptic_gt_folder = panoptic_gt_folder
    ...
    def get_unified_pan_result(self, segs, pans, cls_inds, obj_ids=None, stuff_area_limit=4 * 64 * 64, names=None, num_seg_classes=19, num_classes=9):
        if obj_ids is None:
        ...
        for (seg, pan, cls_ind, obj_id, name) in zip(segs, pans, cls_inds, obj_ids, names):
            ...
            pan_ins = pan.copy()
            pan_obj = pan.copy()
            id_last_stuff = num_seg_classes - num_classes
            ids = np.unique(pan)
            ...
            for idx, id in enumerate(ids_ins):
                ...
                if id - id_last_stuff - 1 >= len(cls_ind):
                    pan_ins[region] = 0
                    pan_obj[region] = 0
                    continue
